...
import type { LaborContractTerminationRecord } from '@/types/labor-contract-termination';

const TEMPLATE_PATH = path.join(process.cwd(), 'src', 'templates', 'labor-contract-termination-template.docx');
...
function fillTemplate(documentXml: string, record: LaborContractTerminationRecord): string {
  const data = record.data;
  let index = -1;
  return documentXml.replace(/<w:p\b[\s\S]*?<\/w:p>/g, (paragraph) => {
    index += 1;
    if (index === 2) {
      return paragraphText(paragraph, `      ${data.employeeName || record.employeeName}${data.honorific || record.honorific}：`);
    }
    if (index === 3) {
      return paragraphText(paragraph, `根据公司与您签订的《劳动合同》，公司自${formatChineseDate(data.terminationDate || record.terminationDate)}起将与您解除劳动聘用关系，终止与您的劳动合同。`);
    }
    if (index === 4) {
      return paragraphText(paragraph, `解除（终止）合同的原因如下：${data.reason || record.reason}`);
    }
    if (index === 5) {
      return paragraphText(paragraph, `请接到本通知后，在${formatChineseDate(data.procedureDeadline || record.procedureDeadline)}前到所属人力资源部门办理相关离职手续。`);
    }
    if (index === 8) {
      return paragraphText(paragraph, `${data.companyName || record.companyName}    `);
    }
    if (index === 10) {
      return paragraphText(paragraph, formatChineseDate(data.noticeDate || record.noticeDate));
    }
    return paragraph;
  });
}

export function buildLaborContractTerminationDocx(record: LaborContractTerminationRecord): Buffer {
