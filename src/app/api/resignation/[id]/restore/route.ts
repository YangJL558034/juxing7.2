...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    `).get(id) as ResignationDbRow | undefined;
    ...
    return NextResponse.json({ success: true, record: parseResignationRow(updated), message: '员工离职申请已恢复' });
  } catch (error) {
    console.error('Restore resignation record error:', error);
    return NextResponse.json({ success: false, error: '恢复员工离职申请失败' }, { status: 500 });
  }
}
